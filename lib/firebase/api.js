...
const savePatient = async({nombre,edad,peso,triage})=>{
    var n=1
    firebase.db.get(firebase.db.ref(d, "/Pacientes")).then((snapshot)=>{
        n=parseInt(snapshot.size)+1
    }).then(()=>{
        firebase.db.set(firebase.db.ref(d, "/Pacientes/Pa"+n),{
            nombre:nombre,
            edad:edad,
            peso:peso,
            triage:triage,
